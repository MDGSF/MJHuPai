...
bool split::bHuNoLaizi(Card aucHandCards[MAX_HANDCARD_NUM])
{
    //34 = 9张万 + ９张条 + 9张筒 + 4张风 + 3张箭
    char cards[34] = { 0 };
    for (int i = 0; i < MAX_HANDCARD_NUM; i++)
    {
        Card ucCard = aucHandCards[i];
        int iIndex = calcCardIndex(ucCard);
        cards[iIndex]++;
    }

    return split::get_hu_info(cards, 34, 34);
}

bool split::bHuHasLaizi(Card aucHandCards[MAX_HANDCARD_NUM], Card ucLaiZi)
{
    //34 = 9张万 + ９张条 + 9张筒 + 4张风 + 3张箭
    char cards[34] = { 0 };
    for (int i = 0; i < MAX_HANDCARD_NUM; i++)
    ...
    return split::get_hu_info(cards, 34, calcCardIndex(ucLaiZi));
}

int split::calcCardIndex(Card ucCard)
{
    static const int iWanStart = 0;
    static const int iTiaoStart = 9;
    static const int iTongStart = 18;
    static const int iFengStart = 27;
    static const int iJianStart = 31;

    int iType = CardType(ucCard);
    int iNum = CardNum(ucCard);
    switch (iType)
    {
    case MJ_TYPE_WAN:
        return iWanStart + iNum - 1;
    case MJ_TYPE_TIAO:
        return iTiaoStart + iNum - 1;
    case MJ_TYPE_BING:
        return iTongStart + iNum - 1;
    case MJ_TYPE_FENG:
        return iFengStart + iNum - 1;
    case MJ_TYPE_JIAN:
        return iJianStart + iNum - 1;
    default:
        //错误
        break;
    }
